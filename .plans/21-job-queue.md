...
- Review batch (baseline 0c81310, threads PRRT_kwDOUQpMs86hyQHn and PRRT_kwDOUQpMs86hyQHr): two timing defects reproduced before fixing. A failed job stayed queued until the 5 s poll ("did not reach state completed" after 2 s), and a claim holding stale `now()` returned undefined for a job that became due while it waited on the claim lock. Fixed with a per-retry wake timer (cleared on stop) and `statement_timestamp()` in the claim predicate; the timer uses the retry duration returned by `queue.fail` so it never depends on synchronized host and database clocks, which the offset-spied regression test reproduced as a failure before the correction. After the fix `bun test apps/server/src/jobs apps/server/src/roles.test.ts` passed 26 tests and 104 assertions, and after the clock-offset correction `bun test apps/server/src/jobs` passed 20 tests and 92 assertions; `bun run --cwd apps/server check`, `bun run --cwd apps/server build` and `bunx biome check apps/server/src/jobs` passed. Evidence: .devin/21-review-fixes.log.
