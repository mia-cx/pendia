...
- PR #51 review fixes restrict group and override mutations to enabled built-in admins, matching the spec's admin-made groups. The admins group row serializes membership replacements; removing the final enabled admin rolls back with CONFLICT. Intentional self-demotion remains possible when another enabled admin exists.
- Permission regressions failed before the fix: manager privilege escalation, sole-admin removal and concurrent admin demotion. After the fix, focused tests pass 10/10; full Postgres tests pass 81/81 with 455 assertions. Lint, typecheck and build pass; without DATABASE_URL, 19 pass and 62 skip. Evidence: .devin/22-permission-red.log and .devin/22-review-validation.log.
- Bootstrap decision: first-caller ownership is acceptable only behind an operator-controlled deployment boundary. Keep fresh instances private until setup completes. The README now states this requirement. Bootstrap secrets and local-only setup are outside this slice.
- Pullfrog's lastSeenAt assertion observation is coverage-only. authenticate writes clock_timestamp() on every matching session update; no missing touch behavior exists. Leave that test unchanged under the review trigger rule.
