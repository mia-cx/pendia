...
- Merge of `origin/main` (PR #53, OIDC and invites) at `f2da8fd`: auth files took main's version with our two exports re-applied on http.ts (`sessionCookieName`, `readSessionToken`), `authCodeMap` gained `INVALID_INVITE`→400 and `OIDC_FAILED`→401 with test rows, package.json took the union of dependencies, and bun.lock was regenerated. Verified `/api/auth/*` still dispatches to the auth handler ahead of the oRPC handler and `authenticate`'s return still matches `Me` exactly.
- Second review round on the pushed head:
  - The batch-level revalidation left a gap: a `yield` suspends until the consumer pulls, so one validation could cover a batch a slow client stretches past the interval. The deadline check moved inside the row loop, ahead of each authorisation. `a498e52`; the regression test `revocation mid-batch stops delivery inside the interval` resolved the second row instead of rejecting before the fix.
  - Per-row authorisation cost a query per replayed row. `memoizeAudience` memoises the decider by event subject (`library:<id>`, one `job` key, `session:<id>` shared by both session kinds) and `subscribe` resets it on every credential refresh, tying authorisation freshness to the same bound. `bad675d`, plus `069c87b` keying unknown kinds without narrowing `never`.
  - Partially accepted: `viewableLibraryIds` now runs its `checkPermission` calls concurrently while keeping the returned ids in library order; the set-based redesign was rejected because it would duplicate the precedence rules. `0f6a3ef`.
  - Personalised GETs answered with no cache directives, so a shared proxy could serve one caller's data to another. Both transports now answer `Cache-Control: no-store` with `Vary: Cookie, Authorization`, matching the auth routes; `/api/openapi.json` stays cacheable. Headers are set on the response oRPC returns, so the event stream's body is never buffered. (pending commit)
- Final gate, all run from the repo root after the second review round: `bun install --frozen-lockfile` clean (116 installs, 219 packages, no changes); `bun run lint` clean (81 files); `bun run check` 6/6 turbo tasks (web svelte-check 0 errors, 0 warnings); `bun run build` 4/4 turbo tasks (server bundle, web static site written); `DATABASE_URL=postgresql://pendia:pendia@127.0.0.1:55423/pendia bun test` 148 pass / 0 fail / 822 expects across 21 files; `bun test` without DATABASE_URL 36 pass / 112 skip / 0 fail with the skip message printed once.
