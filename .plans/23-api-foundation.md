...
- Final gate, all run from the repo root after the non-cacheable fix: `bun install --frozen-lockfile` clean (116 installs, 219 packages, no changes); `bun run lint` clean (81 files); `bun run check` 6/6 turbo tasks (web svelte-check 0 errors, 0 warnings); `bun run build` 4/4 turbo tasks (server bundle, web static site written); `DATABASE_URL=postgresql://pendia:pendia@127.0.0.1:55423/pendia bun test` 151 pass / 0 fail / 841 expects across 21 files; `bun test` without DATABASE_URL 36 pass / 115 skip / 0 fail with the skip message printed once.
