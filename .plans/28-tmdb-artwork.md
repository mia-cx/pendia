...
- Rebase onto new `origin/main` `cf3a7c0`: main claimed migration index 0003 (`0003_nifty_sunspot`, keyframes/timelines). Main's 0003 SQL, snapshot and journal entry were kept; the superseded `0003_nifty_mad_thinker` was removed and `bun run --cwd apps/server db:generate` regenerated the metadata change as `0004_pale_carmella_unuscione` (only the `metadata_state` enum and `items.metadata_state` default `pending`). `db.test.ts` journal assertions moved to 5. Validation runs after the rebase completes.
