...
- [x] 2. Add playback planning and refresh procedures. Load the requested Item and Version, normalize probed Streams, and run decidePlayback with trusted-address LAN detection and persisted bitrate caps. Create a session row and return a token URL only for direct play. Validation: RPC and REST tests cover direct play, non-direct decisions without URLs, input validation, library and play denial, refresh ownership, and registry rows.
...
- TODO 2 validation: `DATABASE_URL=postgresql://pendia:pendia@127.0.0.1:55432/pendia bun test apps/server/src/api/playback.test.ts` passes 12 tests and 51 assertions. Token tests still pass 14 tests after transaction-compatible argument narrowing. Server typecheck and focused Biome checks pass. Planning returns direct URLs over RPC and REST; refresh extends expiry without sleeping. Invalid persisted caps fail closed.
