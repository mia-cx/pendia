...
- [x] Document the routes, auth setting, and exact Authentik provider values for `id.mia.cx`, then run the repository validation gate. Validation: `bun install --frozen-lockfile`, `bun run lint`, `bun run check`, `bun run build`, `DATABASE_URL=postgresql://pendia:pendia@127.0.0.1:55436/pendia bun test`, and `bun test` without `DATABASE_URL` pass with their real results recorded below.
...
- Documentation is complete; live `id.mia.cx` verification is deferred to Mia after merge.
- PR #53 review fixes: UserInfo email and verification are now selected as one pair (a differing UserInfo email without `email_verified` inherits no trust); OIDC URLs derive from the effective trusted-proxy scheme; and invited username collisions retry random suffixes through savepoints instead of racing a pre-check. Focused `bun test apps/server/src/auth/oidc.test.ts` → 12 pass; full gate rerun: lint/check/build clean, `bun test` with DATABASE_URL → 104 pass, 0 fail, 662 expect() calls, without DATABASE_URL → 19 pass, 85 skip, 0 fail. CI exposed a Turbo-cached local typecheck; `pickEmail` now accepts `oauth.IDToken` directly. No-cache results: `bun run --cwd apps/server check` and `bun x turbo run check --force` both clean.
- TODO 5 verified: `bun install --frozen-lockfile` → 92 installs across 196 packages, no changes, exit 0; `bun run lint` → 64 files checked, 0 errors, exit 0; `bun run check` → 5 tasks successful, exit 0; `bun run build` → 4 tasks successful, exit 0; `DATABASE_URL=postgresql://pendia:pendia@127.0.0.1:55436/pendia bun test` → 101 pass, 0 fail, 643 expect() calls across 15 files, exit 0; `env -u DATABASE_URL bun test` → 19 pass, 82 skip, 0 fail, 49 expect() calls, exit 0.
