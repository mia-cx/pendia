/** What a medium hands the core: an in-tree module per medium behind this interface. */

/** Core Home shelves a medium's Items can join. */
export type CoreShelf =
  | "continue-watching"
  | "recently-added"
  | "recently-played";

/** Everything the core needs from a medium. */
export interface Medium {
  ...
  translation?: {
    protocol: "jellyfin" | "opensubsonic" | "opds" | "hdhomerun";
  };
}

/** One Item kind a medium owns. */
...
/** Rules for assigning library paths to this medium during a scan. */
...
/** Shelves and client routes a medium adds to browse. */
export interface BrowseContribution {
  ...
  shelves: {
    id: string;
    title: string;
    items(context: { userId: string }): Promise<string[]>;
  }[];
